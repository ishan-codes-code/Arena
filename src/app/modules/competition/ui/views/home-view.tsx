    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
        ...
        <section className="mx-auto grid w-full max-w-[1440px] gap-8 px-4 pb-12 pt-8 sm:px-6 sm:pt-12 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)] lg:gap-12 lg:px-10 lg:pb-16 lg:pt-16">
          <div className="flex min-w-0 flex-col justify-between gap-10">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <a href="#tournaments" className="inline-flex min-h-12 items-center justify-center gap-3 bg-primary px-5 font-body text-xs font-bold uppercase tracking-[0.12em] text-primary-foreground transition-transform hover:-translate-y-0.5">Find your match <ArrowUpRight className="size-4" /></a>
              <span className="flex items-center gap-2 font-body text-[10px] uppercase tracking-[0.14em] text-muted-foreground"><ShieldCheck className="size-4 text-live" /> No entry fees. Ever.</span>
            </div>
          </div>
          ...
          <div className="arena-cut-panel-tight relative overflow-hidden border border-border-strong bg-card p-4 shadow-[12px_12px_0_var(--border)] sm:p-6">
            ...
            <Link href="/signup" className="flex min-h-12 items-center justify-between border border-border-strong px-4 font-body text-xs font-bold uppercase tracking-[0.12em] transition-colors hover:bg-primary hover:text-primary-foreground"><span>Lock my slot</span><ChevronRight className="size-4" /></Link>
          </div>
        </section>
      </div>
      ...
      <section className="border-y border-border bg-card" id="tournaments">
        <div className="mx-auto w-full max-w-[1440px] px-4 py-10 sm:px-6 sm:py-14 lg:px-10">
          <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-3 font-mono text-[10px] uppercase tracking-[0.2em] text-primary">The room list</p><h2 className="font-display text-4xl font-black tracking-[-0.06em] sm:text-5xl">Pick your pressure.</h2></div><Link href="/tournaments" className="flex items-center gap-2 font-body text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground hover:text-foreground">View all rooms <ArrowUpRight className="size-4" /></Link></div>
      ...
      <footer className="border-t border-border bg-card px-4 py-7 sm:px-6 lg:px-10"><div className="mx-auto flex w-full max-w-[1440px] flex-col justify-between gap-3 font-body text-[10px] uppercase tracking-[0.12em] text-muted-foreground sm:flex-row"><span>Built for everyday players · Free entry, always.</span><span>Not affiliated with or endorsed by Garena.</span></div></footer>
    </main>
  );
}
